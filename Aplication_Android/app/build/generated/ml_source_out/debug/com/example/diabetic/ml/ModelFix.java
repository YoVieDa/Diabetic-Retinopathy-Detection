package com.example.diabetic.ml;
...
public final class ModelFix {
  ...
  private ModelFix(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "Model_fix.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static ModelFix newInstance(@NonNull Context context) throws IOException {
    return new ModelFix(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static ModelFix newInstance(@NonNull Context context, @NonNull Model.Options options)
      throws IOException {
    return new ModelFix(context, options);
  }
